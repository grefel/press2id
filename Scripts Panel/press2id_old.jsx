...
function processDok(dok) {
	...
	if (px.showGUI) {
		...
		var selectedPostsArray = px.debugPost;
	}

	if (selectedPostsArray == null) {
	...
	for (var r = 0; r < selectedPostsArray.length; r++) {
		...
		var endPoint = selectedPost.endPoint

		blogURL = discoverRestUrl(blogURL);
		...
		try {
			...
			if (px.renderMode == "flow") {
				...
				var singlePost = getSingleEntity(blogURL, endPoint, postObject);
				if (singlePost == null) {
					continue;
				}
				...
				if (r == 0) {
					...
					try {
						var styleTemplateDok = app.open(templateFile, px.debug, OpenOptions.OPEN_COPY);
					}
					catch (e) {
				...
				var startPage = styleTemplateDok.pages[-1]; //.add({appliedMaster:mspread});
					...
					for (var i = 0; i < imgArray.length; i++) {
						...
						if (imageFile.exists && imageFile.length > 0) {
							...
							if (imgXML.markupTag.name == "featuredImage") {
								...
								rect = styleTemplateDok.rectangles.add();
				...
				var singleACFBlock = getSingleEntity(blogURL, endPoint, postObject);
...
function getConfig() {
	...
	ui.staticTextendPointDescription2 = { en: "Posts, Pages or Custom Post Types", de: "Beiträge, Seiten oder Custom Post Types" };
	...
	ui.panelProcessMode = { en: "Process Mode", de: "Verarbeitungsmodus" };
	ui.radioProcessFlow = { en: "Render Textflow", de: "Als Textfluss darstellen" };
	ui.radioProcessTemplate = { en: "Fill a template", de: "Eine Vorlage befüllen" };
	...
	var endPointArray = getendPoints(blogURL);
	var endPoint = endPointArray[0];
	var listItems = [];
	listItems = getListOfBlogEntries(blogURL, 1, false, endPoint, px.beforeDate, px.afterDate);
	...
	buttonBlogInfoFetch.onClick = function () {
		...
		if (blogURL == "") return;
		endPointArray = getendPoints(edittextBlogInfoURL.text);

		var setNewEndpoint = true;
		endPointDropdown.removeAll();
		for (var f = 0; f < endPointArray.length; f++) {
			endPointDropdown.add("item", endPointArray[f]);
			if (endPoint == endPointArray[f]) {
		...
		listItems = getListOfBlogEntries(blogURL, 5, true, endPoint, px.beforeDate, px.afterDate);
	...
	var groupendPoint = panelBlogInfo.add('group');
	groupendPoint.add('statictext {text: "' + localize(ui.staticTextendPointDescription) + '"}');

	var endPointDropdown = groupendPoint.add("dropdownlist", undefined, endPointArray);
	groupendPoint.add('statictext {text: "' + localize(ui.staticTextendPointDescription2) + '"}');

	endPointDropdown.selection = 0;
	endPointDropdown.preferredSize.width = 90;
	endPointDropdown.preferredSize.height = 24;

	endPointDropdown.onChange = function () {
		if (endPoint != endPointDropdown.selection.text) {
			log.info("Change endpoint to " + endPointDropdown.selection.text);
			endPoint = endPointDropdown.selection.text;
			buttonBlogInfoFetch.onClick();
		}
	}
	...
	if (win.show() != 2) {
		...
		if (canRun()) {
			...
			if (radioProcessFlow.value) {
				px.renderMode = "flow";
			}
			if (radioProcessTemplate.value) {
				px.renderMode = "template";
			}
 ...
 */
function getListOfBlogEntries(fixedURL, maxPages, verbose, endPoint, beforeDate, afterDate) {
	var listItems = [];
	var ui = {};
	...
	for (var page = 1; page <= maxPages; page++) {
		...
		var action = endPoint + "/?_fields[]=title&_fields[]=id&per_page=100&page=" + page + "&before=" + beforeDate + "T00:00:00&after=" + afterDate + "T00:00:00";
		...
		if (verbose && postEmbed.length == 0 && listItems.length == 0 && response.httpStatus == 200) {
			log.infoAlert(localize(ui.noBlogPostsOnSite, blogURL, endPoint));
			return;
		}
	}
